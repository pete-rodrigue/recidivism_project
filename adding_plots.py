...
#do data exploration here
# do it on crimes_w_demographic

#describing numeric columns
crimes_w_demographic.columns

bg_ml.na_summary(crimes_w_demographic)

#Based on these N/As, we would impute height, weight (continuous)
...
describe = bg_ml.describe_data(crimes_w_demographic, vars_to_describe=vars_to_describe)
describe

bg_ml.histograms(crimes_w_demographic, vars_to_describe=vars_to_describe)


correlations = bg_ml.correlations(crimes_w_demographic, vars_to_describe=vars_to_describe)
bg_ml.correlation_matrix(correlations)

#pairplot

bg_ml.boxplots(crimes_w_demographic, vars_to_describe=vars_to_describe)
...
summary_table_race = col_aggregation(crimes_w_demographic, 'OFFENDER_RACE_CODE')
summary_table_race

summary_table_gender = col_aggregation(crimes_w_demographic, 'OFFENDER_GENDER_CODE')
summary_table_gender

summary_table_ethnic = col_aggregation(crimes_w_demographic, ['OFFENDER_RACE_CODE','OFFENDER_ETHNIC_CODE'])
summary_table_ethnic

summary_table_citizen = col_aggregation(crimes_w_demographic, 'OFFENDER_CITIZENSHIP_CODE')
summary_table_citizen

summary_table_felony_misd = col_aggregation(crimes_w_demographic, 'crime_felony_or_misd')
summary_table_felony_misd


# explain why we drop certain variables
# why don't I see the variables in list_of_vars_to_make_count_vars_with in the overall dataset?
# that means I can't do data exploration on it
# can that be fixed?
